...
<template v-slot:[`item.req`]="{ item }">
      ...
      <template v-slot:no-data>
        <span>Asgje per te shfaqur</span>
      </template>
    </v-data-table>
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-form ref="form" v-model="validLogin" lazy-validation>
          <v-card-title>
            <span class="text-h5">Per te vazhduar tutje, ju lutem kyquni!</span>
          </v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
            ...
            <small>*te gjitha fushat jane te kerkuara</small>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "Receta", align: "start", value: "name" },
      { text: "Krijuesi i recetes", value: "creator" },
      { text: "Perberesit", value: "req" },
    ...
    availableFoods: ["Miell", "Kripe", "Sheqer", "Uje", "Mish", "Veze"],
    allRecipes: [],
  }),
    ...
    login() {
      ...
      if (this.username === "admin" && this.password === "npost") {
        ...
        this.message = {
          ...this.message,
          state: true,
          content: "Keni gabuar username-in ose password-in",
          icon: "mdi-check-circle",
          color: "error",
        };
      }
